...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  const [localData, setLocalData] = useLocalStorage<AppData>(
    "m13-offline-data",
    initialData
  );
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const cloudHasData =
        !countError && existingPeople && existingPeople.length > 0;
      ...
      const cloudData: AppData = {
        ...
        villageEntries: extractData(villageRes, "village_entries").map(
          (v: any) => ({
            id: v.id,
  ...
  const uploadLocalDataToCloud = async () => {
    ...
    try {
      ...
      if (localData.villageEntries && localData.villageEntries.length > 0) {
        const villageEntriesToUpload = localData.villageEntries.map(
          (entry) => ({
            id: entry.id,
            user_id: user.id,
        ...
        if (villageError) {
          ...
          console.log(
            `✅ Uploaded ${villageEntriesToUpload.length} village entries`
          );
        }
      }
      ...
      if (localData.dairyEntries && localData.dairyEntries.length > 0) {
        ...
        if (dairyError) {
          ...
          console.log(
            `✅ Uploaded ${dairyEntriesToUpload.length} dairy entries`
          );
        }
      }
  ...
  const updateVillageEntry = async (
    id: string,
    updatedEntry: Partial<VillageEntry>
  ) => {
    if (!user) return;

    setData((prev) => ({
      ...prev,
      villageEntries: prev.villageEntries.map((e) =>
  ...
  const updateCityEntry = async (
    id: string,
    updatedEntry: Partial<CityEntry>
  ) => {
    if (!user) return;

    setData((prev) => ({
      ...prev,
      cityEntries: prev.cityEntries.map((e) =>
        e.id === id ? { ...e, ...updatedEntry } : e
      ),
    }));
  ...
  const updateDairyEntry = async (
    id: string,
    updatedEntry: Partial<DairyEntry>
  ) => {
    if (!user) return;

    setData((prev) => ({
      ...prev,
      dairyEntries: prev.dairyEntries.map((e) =>
        e.id === id ? { ...e, ...updatedEntry } : e
      ),
    }));
